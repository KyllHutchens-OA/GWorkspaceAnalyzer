...
interface FreeTierBannerProps {
  ...
  lockedIssuesValue?: number;
}

export function FreeTierBanner({ scansUsed, scansLimit, lockedIssuesCount = 0, lockedIssuesValue = 0 }: FreeTierBannerProps) {
        ...
        <div className="flex-1">
          ...
          {lockedIssuesCount > 0 && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20">
              ...
              <p className="text-sm">
                <span className="font-bold">{lockedIssuesCount} more {lockedIssuesCount === 1 ? 'issue' : 'issues'}</span> worth <span className="font-bold">${lockedIssuesValue.toLocaleString()}</span> detected. Upgrade to unlock.
              </p>
            </div>
          )}
        </div>

        <Button variant="outline" size="lg" className="whitespace-nowrap bg-white text-emerald-600 border-white hover:bg-gray-50">
          Upgrade Now
        </Button>
      </div>
    </div>
