...
def generate_sample_invoices(user_id: str, scan_job_id: str) -> List[Dict[str, Any]]:
...
def generate_sample_findings(user_id: str, invoices: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for other in vendor_invoices[i+1:]:
                ...
                if days_diff <= 7 and abs(invoice["amount"] - other["amount"]) < 1:
                    findings.append({
                        ...
                        "severity": "high",
                        "confidence_score": 0.95,
                        "potential_savings": invoice["amount"],
                        "metadata": {
                            "vendor": vendor,
    ...
    for vendor in sprawl_vendors:
        if vendor in by_vendor and len(by_vendor[vendor]) >= 3:
            findings.append({
                "user_id": user_id,
                "finding_type": "subscription_sprawl",
                "status": "new",
...
async def seed_development_data(
    ...
    invoices = generate_sample_invoices(user_id, scan_job_id)
    ...
    findings = generate_sample_findings(user_id, invoices)
